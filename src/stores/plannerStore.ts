  ...
  return WALLS.map((w) => {
    ...
    return isVert
      ? {
          x0: w.x1 - T,
          x1: w.x1 + T,
          z0: Math.min(w.z1, w.z2),
          z1: Math.max(w.z1, w.z2),
        }
      : {
...
function rectsOverlap(a: Rect, b: Rect): boolean {
  return (
    a.x0 + OVERLAP_EPS < b.x1 &&
    a.x1 - OVERLAP_EPS > b.x0 &&
    a.z0 + OVERLAP_EPS < b.z1 &&
    a.z1 - OVERLAP_EPS > b.z0
  );
}
...
function snapToWall(
  ...
  for (const wall of WALLS) {
    ...
    if (isVert) {
      ...
      const inRange =
        rawZ >= Math.min(wall.z1, wall.z2) &&
        rawZ <= Math.max(wall.z1, wall.z2);
      ...
      const inRange =
        rawX >= Math.min(wall.x1, wall.x2) &&
        rawX <= Math.max(wall.x1, wall.x2);
      const score = perpDist + (inRange ? 0 : 0.001);
      if (score < bestScore) {
...
function resolveMovement(
  ...
  for (const other of allFurniture) {
    if (
      other.uid === item.uid ||
      APERTURE_IDS.has(other.catalogId) ||
    ...
    if (gxr >= 0 && gxr < SNAP_DIST) {
      ...
      if (
        !WALL_RECTS.some((wr) =>
          rectsOverlap(furnitureRect(cx, newZ, item), wr),
        )
      )
        newX = cx;
    }
    ...
    if (gzb >= 0 && gzb < SNAP_DIST) {
      ...
      if (
        !WALL_RECTS.some((wr) =>
          rectsOverlap(furnitureRect(newX, cz, item), wr),
        )
      )
        newZ = cz;
    }
  ...
  const wallClamp = (pos: number, isX: boolean, crossPos: number): number => {
    for (const wr of WALL_RECTS) {
      const r = isX
        ? furnitureRect(pos, crossPos, item)
        : furnitureRect(crossPos, pos, item);
  ...
  const furnCollides = (cx: number, cz: number): boolean =>
    allFurniture.some((f) => {
      if (
        f.uid === item.uid ||
        APERTURE_IDS.has(f.catalogId) ||
        !heightsOverlap(item, f)
      )
        return false;
      return rectsOverlap(
        furnitureRect(cx, cz, item),
        furnitureRect(f.x, f.z, f),
      );
    });

  let resolvedX = WALL_RECTS.some((wr) =>
    rectsOverlap(furnitureRect(newX, item.z, item), wr),
  )
    ? wallClamp(newX, true, item.z)
    : newX;
  if (furnCollides(resolvedX, item.z)) resolvedX = item.x;

  let resolvedZ = WALL_RECTS.some((wr) =>
    rectsOverlap(furnitureRect(resolvedX, newZ, item), wr),
  )
...
export const usePlannerStore = defineStore("planner", () => {
  ...
  if (stored) {
    try {
      placedFurniture.value = JSON.parse(stored);
    } catch {
      /* ignore corrupt data */
    }
  }
  ...
  async function exportPlan(): Promise<void> {
    const payload = JSON.stringify(
      { version: 1, furniture: placedFurniture.value },
      null,
      2,
    );
  ...
  function loadPlanData(data: any): void {
    try {
      if (data && Array.isArray(data.furniture)) {
        placedFurniture.value = data.furniture;
        selectedUid.value = null;
      } else if (Array.isArray(data)) {
        placedFurniture.value = data;
        selectedUid.value = null;
      }
      console.log("Piano caricato con successo");
    } catch (err) {
      console.error("Errore nel caricamento del piano predefinito:", err);
    }
  }
